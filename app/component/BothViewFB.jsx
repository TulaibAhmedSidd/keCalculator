...
// --- Firestore helpers ---
const saveKEData = async (uid, keData) => {
  try {
    console.log("saveKEData",)

    const keRef = await doc(db, "users", uid, "ke_data", "latest");
    console.log("keRef", keRef)
    await setDoc(keRef, keData, { merge: true });
    console.log("setDoc")
  } catch (error) {
    console.log("error saveKEData", error)
  }
};

const getKEData = async (uid) => {
  try {
    const keRef = doc(db, "users", uid, "ke_data", "latest");
    const snapshot = await getDoc(keRef);
...
const getIndigoInputClasses = (isDarkMode) =>
  `w-full p-3 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition duration-150 ${isDarkMode
...
const MultiApplianceView = ({
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div className="lg:col-span-2">
      ...
      <button
        disabled={ilaosder}
        onClick={handleAddnSaveAppliance}

        className={`
            ${ilaosder
          && 'bg-gray-400 text-gray-200 cursor-not-allowed'
          }
          w-full bg-indigo-600 text-white p-3 rounded-lg font-semibold shadow-md hover:bg-indigo-700 transition duration-150 flex items-center justify-center space-x-2`}
      >
        <PlusIcon className="w-5 h-5" />
...
const BothView = () => {
  ...
  const [ilaosder, setIslaoder] = useState(false);
  const [appliances, setAppliances] = useState([]);
  const [pricePerKWh, setPricePerKWh] = useState(49.33);
  const [fixedCharge, setFixedCharge] = useState(22.44);
  const [variableTaxPerKWh, setVariableTaxPerKWh] = useState(0.00);
  ...
  useEffect(() => {
    const unsubscribe = onAuthChange(currentUser => {
      ...
      if (!currentUser) setShowLogin(true);
        ...
        // Load last saved KE data
        getKEData(currentUser.uid).then(data => {
          console.log("data getKEData", data)
          if (!data) return;
          setAppliances(data.appliances || []);
          setNewName(data.newName || '');
          setNewWatts(data.newWatts || '');
          setNewHours(data.newHours || '');
          setPricePerKWh(data.pricePerKWh || 49.33);
          setFixedCharge(data.fixedCharge || 22.44);
          setVariableTaxPerKWh(data.variableTaxPerKWh || 0);
          setDaysInMonth(data.daysInMonth || 30);
          setIsModeSingle(data.mode === 'single');
        }).then(() => {
          setIslaoder(false)
        }).catch((e) => {
          console.log("e", e)
  ...
  const getNewData = async () => {
    setIslaoder(true)
    await getKEData(user.uid).then(data => {
      console.log("data getKEData", data)
      if (!data) return;
      setAppliances(data.appliances || []);
      setNewName(data.newName || '');
  ...
  console.log("ilaosder", ilaosder)

  // // --- Firebase Save KE data ---
  // useEffect(() => {
  //   if (!user) return;
  //   console.log("im  wporking",)
  //   const dataToSave = {
  //     mode: isModeSingle ? 'single' : 'multi',
  //     appliances,
  //     newName,
  //     newWatts,
  //     newHours,
  ...
  //   };
  //   saveKEData(user.uid, dataToSave);
  // }, [appliances, newName, newWatts, newHours, isModeSingle, pricePerKWh, fixedCharge, variableTaxPerKWh, daysInMonth, user]);

  // --- Multi-Appliance CRUD (unchanged) ---
  const handleAddAppliance = (e) => {
    e.preventDefault();
    const wattsValue = parseFloat(newWatts);
    const hoursValue = parseFloat(newHours);
    if (isNaN(wattsValue) || isNaN(hoursValue) || wattsValue <= 0 || hoursValue < 0) return;
    const newAppliance = { id: crypto.randomUUID(), name: newName || `Appliance ${appliances.length + 1}`, watts: wattsValue, hours: hoursValue };
    setAppliances(prev => [...prev, newAppliance]);


    setNewName(''); setNewWatts(''); setNewHours('');
  };
  const handleAddnSaveAppliance = async () => {
    if (!user) return;
    setIslaoder(true)
    console.log("im  wporking",)
    const dataToSave = {
    ...
    await saveKEData(user.uid, dataToSave).then(() => {
      setIslaoder(false)
    }).catch((e) => {
      console.log("e", e)
      setIslaoder(false)
  ...
  const handleDeleteAppliance = useCallback((id) => setAppliances(prev => prev.filter(app => app.id !== id)), []);
  ...
  const calculation = useMemo(() => {
    ...
    itemsToProcess.forEach(item => {
      ...
      if (watts > 0 && hours >= 0) {
        ...
        calculatedItems.push({ id, name, watts, hours, dailyKWh: dailyKWh.toFixed(2), dailyCost: (dailyKWh * energyRate).toFixed(2) });
      }
    });
    ...
    return {
      total: { dailyKWh: totalDailyKWh.toFixed(2), dailyEnergyCost: (totalDailyKWh * energyRate).toFixed(2), totalKWh: totalKWh.toFixed(2), monthlyEnergyCost: monthlyEnergyCost.toFixed(2), monthlyVariableTax: monthlyVariableTax.toFixed(2), fixedChargeCost: fixedRate.toFixed(2), totalBillCost: totalBillCost.toFixed(2), days },
  ...
  const handleEmailLogin = async () => {
    try { await loginWithEmail(email, password); setShowLogin(false); }
    catch (e) { alert("Login failed: " + e.message); }
  };
  const handleregisterWithEmail = async () => {
    try { await registerWithEmail(email, password); setShowLogin(false); }
    catch (e) { alert("Login failed: " + e.message); }
  };
  const handleGoogleLogin = async () => { try { await loginWithGoogle(); setShowLogin(false); } catch (e) { alert("Login failed: " + e.message); } };
  // --- Logout function ---
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-900">
        <div className="bg-gray-800 p-8 rounded-xl shadow-lg w-96 text-center">
          ...
          <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} className="w-full mb-3 p-3 border rounded-lg" />
          <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="w-full mb-3 p-3 border rounded-lg" />
          <button onClick={handleEmailLogin} className="w-full mb-2 bg-indigo-600 text-white p-3 rounded-lg">Login with Email</button>
          <button onClick={handleregisterWithEmail} className="w-full mb-2 bg-indigo-600 text-white p-3 rounded-lg">Register with Email</button>
          <button onClick={handleGoogleLogin} className="w-full bg-red-500 text-white p-3 rounded-lg">Register with Google</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={` overflow-hidden min-h-screen p-4 sm:p-8 font-inter transition-colors duration-500 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className={`w-full max-w-5xl mx-auto p-6 sm:p-10 rounded-2xl shadow-2xl transition-colors duration-500 ${isDarkMode
        ...
        <header className="mb-8 relative ">
          ...
          <div className="flex items-center justify-end gap-4 mb-4">
            {user && (
              <div className="flex items-center gap-2 bg-gray-700 text-white px-3 py-1 rounded-full">
                {/* First letter avatar */}
            ...
            <button
              ...
              className={`p-3 rounded-full transition duration-300 ${isDarkMode
        ...
        {isModeSingle ? (
          ...
          <MultiApplianceView
            ...
            totalConsumption={totalConsumption}
            handleAddnSaveAppliance={handleAddnSaveAppliance}
