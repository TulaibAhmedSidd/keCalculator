...
const BothView = ({user}) => {
  ...
  const loadUserData = () => {
    const storedData = JSON.parse(localStorage.getItem(`user_${user.email}`) || '{}');
    return {
      appliances: storedData.appliances || [],
      pricePerKWh: storedData.pricePerKWh || 49.33,
      fixedCharge: storedData.fixedCharge || 22.44,
      variableTaxPerKWh: storedData.variableTaxPerKWh || 0.0,
      daysInMonth: storedData.daysInMonth || 30,
    };
  };
  ...
  const [appliances, setAppliances] = useState(initialData.appliances);
  const [pricePerKWh, setPricePerKWh] = useState(initialData.pricePerKWh);
  const [fixedCharge, setFixedCharge] = useState(initialData.fixedCharge);
  const [variableTaxPerKWh, setVariableTaxPerKWh] = useState(initialData.variableTaxPerKWh);
  ...
  const handleAddAppliance = (e) => {
    ...
    saveUserData({ appliances: updatedAppliances });
  ...
  const handleConfigChange = (setter, key) => (value) => {
    setter(value);
    ...
    itemsToProcess.forEach(item => {
      ...
      if (watts > 0 && hours >= 0) {
        ...
        calculatedItems.push({
          id, name, watts, hours,
          dailyKWh: dailyKWh.toFixed(2),
          dailyCost: (dailyKWh * energyRate).toFixed(2),
        });
      }
    });
